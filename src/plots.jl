export get_cycles, slipplot


"""
    slipplot(dirname::String, startfinish::Tuple{Integer, Integer}, spacing::Tuple{AbstractFloat, AbstractFloat})

Creates and displays a cummulative slip plot from directory `dirname` that begins at `start_cycle`, and ends at `finish_cycle`.
"""
function slipplot(dirname::String, startfinish::Tuple{Integer, Integer}, spacing::Tuple{AbstractFloat, AbstractFloat})
    ...
    for i in 1:length(inds)-1
        ...
        interp = interpolate((depth, t_cycle), δ_cycle, (Gridded(Linear()), Gridded(Linear())))
